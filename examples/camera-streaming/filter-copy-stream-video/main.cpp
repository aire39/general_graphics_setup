...
int32_t main(int32_t argc, char*argv[])
{
  ...
  while (is_running)
  {
    ...
    if (const std::shared_ptr<FImage> frame = camera->CopyFrame())
    {
      frame->UpdateImageFilter(frame->GetFinalImageFilterID());

      // this works because the camera_menu assumes the incoming frame is independent of the camera
      // which is true since the frame is copied and not extracted
      camera_menu.UpdateFrame(frame.get());
