...
int32_t main(int32_t argc, char*argv[])
{
  ...
  bool time_switch = true;
  bool camera_switch = true;
  auto camera_switch_thread = cthread("timing", "switch camera on/off", [&]() {

    auto time_reached = std::chrono::steady_clock::now();
    const auto period = std::chrono::seconds(1u);
    uint32_t counter = 0;
    ...
    while (is_running)
    {
      ...
      time_reached += period;
      std::this_thread::sleep_until(time_reached);
    }
  });
  ...
  if (camera_switch_thread.joinable())
  {
    camera_switch_thread.join();
  }
