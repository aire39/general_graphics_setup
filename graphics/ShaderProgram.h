...
class ShaderProgram
{
  public:
    ShaderProgram();
    explicit ShaderProgram(const OGLShader & shader_a, const OGLShader & shader_b);
    ~ShaderProgram();
    ...
    void SetFloat4x4(const std::string &name, const float* value, bool transpose = false) const;
